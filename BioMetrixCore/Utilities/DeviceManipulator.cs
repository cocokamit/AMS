    ...
    internal class DeviceManipulator
    {
        ...
        public ICollection<MachineInfo> GetLogData(ZkemClient objZkeeper, int machineNumber)
        {
            ...
            while (objZkeeper.SSR_GetGeneralLogData(machineNumber, out dwEnrollNumber1, out dwVerifyMode, out dwInOutMode, out dwYear, out dwMonth, out dwDay, out dwHour, out dwMinute, out dwSecond, ref dwWorkCode))
            {
                ...
                else if (dwInOutMode == 1)
                {
                    objInfo.type = "O";
                }
                else if (dwInOutMode == 2 || dwInOutMode == 5)
                {
                    objInfo.type = "Out";
                }
                else if (dwInOutMode == 3 || dwInOutMode == 4)
                {
                    objInfo.type = "In";
                }
                else
                ...
                if (sdwEnrollNumber != sLastEnrollNumber)
                    {
                        ...
                        {
                            if (idwFingerIndex.ToString() != "" && iFlag.ToString() != "" && sTmpData != "")
                            {
                                ddd = objZkeeper.SetUserTmpExStr(machineNumber, sdwEnrollNumber, idwFingerIndex, iFlag, sTmpData);//upload templates information to the memory
                            }
                        }
                        //else return false;
                    }
