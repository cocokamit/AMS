...
namespace BioMetrixCore
{
    public class ZkemClient : IZKEM
    {
        ...
        public static Master mainForm;
        public static Button button,buttonOnSave;
        public String ipaddress;
        public String Machinenumber;
        
        CZKEM objCZKEM = new CZKEM();
        public ZkemClient(Action<object, string> RaiseDeviceEvent,String ipaddress,String Machinenumber)
        {
        ...
        {
            string ss = ipaddress;
            string mn = Machinenumber;
            DataTable dtbio = dbhelper.getdatahris("Select * from BranchBioDevicesEmp where empid=" + EnrollNumber + " and deviceid=(Select Id from BranchBioDevices where IpAddress='" + ss + "' and Id='" + mn + "')");
            DataTable bio = dbhelper.getdatahris("Select * from BranchBioDevicesEmp where empid=" + EnrollNumber + "");
            DataTable location = dbhelper.getdatahris("Select * from BranchBioDevices where IpAddress='" + ss + "'");
            if (dtbio.Rows.Count > 0)
            {
                string inputDate = new DateTime(Year, Month, Day, Hour, Minute, Second).ToString("yyyy-MM-dd HH:mm:ss");
                DateTime pdate = DateTime.Parse(inputDate);
                string datef = DateTime.Now.ToString("yyyy-MM-dd 00:00:00");
                string datel = DateTime.Parse(datef).AddHours(23).ToString("yyyy-MM-dd HH:mm:ss");
                string dateb = DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd 00:00:00");
                string type = "";
                ...
                else if (AttState == 2 || AttState == 5)
                {
                    type = "Out";
                }
                else if (AttState == 3 || AttState == 4)
                {
                    type = "In";
                }
                else
                ...
                DataTable dt = new DataTable();
                    ...
                    sc.sa = EnrollNumber;
                    sc.sb = pdate.ToString();
                    sc.sc = type=="In"?"Break In": type=="Out"?"Break Out":type;
                    sc.sd = "1";
          
                    dt = dbhelper.getdatahris("Select * from MEmployee where BiometricIdNumber='" + EnrollNumber + "'");

                    if (dt.Rows.Count > 0)
                    {
                    string x = bol.addCheckinout(sc);
                    DataTable dtt = dbhelper.getdatahris("Select * from tdtrperpayrolperline where dtrperpayrol_id=0 and empid='" + dt.Rows[0]["Id"].ToString() + "' and  idnumber='" + dt.Rows[0]["IdNumber"].ToString() + "' and biotime='" + pdate.ToString() + "' and checktype='" + type + "'");
                        if (dtt.Rows.Count == 0)
                        {
                            dt = dbhelper.getdatahris("Insert into tdtrperpayrolperline(dtrperpayrol_id,empid,idnumber,biotime,checktype) values(0,'" + dt.Rows[0]["Id"].ToString() + "','" + dt.Rows[0]["IdNumber"].ToString() + "','" + pdate.ToString() + "','" + type + "')  SELECT  SCOPE_IDENTITY() ids ");
                            dbhelper.getdatahris("Insert into tdtrperpayrol_LogLocation(Location,tdtrid) values('" + location.Rows[0]["BranchName"].ToString() + "'," + dt.Rows[0]["ids"].ToString() + ")");
                        }
                    }
                button.PerformClick();
            }
